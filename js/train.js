...
function buildSVGTrack() {
  ...
  const dur = (trackLen / 120).toFixed(1);
...
function buildAccessorySVG(cx, cy, rx, ry, unlockedSc) {
  const accessories = unlockedSc.filter(id => {
...
function renderShop() {
  ...
  SHOP_SECTIONS.forEach(({ label, filter }) => {
    ...
    const shopPieces = pieces.filter(p => {
      if (p.isRollingStock || p.isTrack) return true;
      return !unlockedSc.includes(p.id) && !p.starter;
    });
